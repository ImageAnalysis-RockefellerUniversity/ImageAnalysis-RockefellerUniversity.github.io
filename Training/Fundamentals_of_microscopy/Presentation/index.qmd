...
format:
  revealjs:
    ...
    width: 1280
    height: 720
    background-image: images/watermark_v8.png
    ...
    footer: "Fundamentals of Microscopy workshop 2026&emsp;-&emsp;Bio-Imaging Resource Center, The Rockefeller University"
    # theme: dark
...
## Sensor vs Detector vs Camera {.custom_indent4 visibility="hidden"}
...
- This wavelength dependent conversion of light to photoelectrons is specified as  the [quantum efficiency (QE)]{.text-red}.
...
![](images/CCD_image%20sensor2.jpeg){.absolute top=0 left=850 width="30%"}
![](images/SS_QE_wavelength_dependence.png){.absolute top=460 left=850 width="30%" .fragment}
...
::: {.smallerFont45 .absolute top="620" left="5"}
[Resource]{.underline}  
...
- Noise is not the same as background
  - [Noise]{.text-red}: random corruption of any image due to inherent uncertainty of photon detection
  - [Background]{.text-red}: any detected signal that is [biologically]{.underline} non-informative

:::

::: {.element-center .smallerFont45}
![](images/SS_SNR_equation3.png)
...
- [Sample dependent]{.text-red}, not camera dependent.
...
::: {.smallerFont55 .absolute top="700" left="5"}
...
![](images/SS_Nat_Meths_paper.png){fig-align="center"}
...
- Various AI-denoising methods have been developed by training deep learning models on:
  - pairs of noisy (low SNR) and clean (high SNR) images 
    - [CARE]{.text-red} (Content-Aware Image Restoration)
    - [RCAN]{.text-red} (Residual Channel Attention Networks, from AIVIA)
  - pairs of noisy images - [Noise2Noise]{.text-red} (N2N)
  - Single noisy images - [Noise2Void]{.text-red} (N2V)

:::{.fragment}
- Problems with AI-denoising:
  - AI can [create, alter, or hallucinate]{.text-red} features that do not exist in the raw data, producing images that look realistic but are scientifically inaccurate.
  - [Resource-intensive]{.text-red}: model training requires hundreds to thousands of paired (noisy/clean) images to train effectively
  - Some methods such as Noise2Void fail to remove structured noise
:::
:::

::: {.element-center}
![](images/CARE_N2N_N2V_comparison.jpg)

<br>
![](images/N2V_structured%20noise2.png){.fragment}
...
:::{.smallerFont65}
- A mathematical operation that reverses the effects of convolution (blurring due to out of focus light) in an image, using the [point spread function (PSF)]{.text-red} of the imaging system.
- Does not require deep learning model training, so it is [less computationally-intensive]{.text-red} than AI-denoising
- It does require accurate estimation of the PSF for optimal results.
- Deconvolution can improve image resolution and contrast without introducing hallucinations, as it is based on the [physical properties of the imaging system]{.text-red} rather than learned patterns from data.
...
::: {.element-center .smallerFont45}
![](images/Convolution_Huygens.png){width="50%"}
![](images/CARE_vs_Huygens_deconvolution.jpg){.fragment}

:::
:::

::: {.smallerFont0 .absolute top="620" left="10"}
[Resource]{.underline}  
https://svi.nl/AI-Image-Denoising
:::

## File format {visibility="hidden"}
...
## Resources {.smaller visibility="hidden"}
